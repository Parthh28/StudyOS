        ...
        {subjectStats.map((stat: any) => {
          ...
          return (
            <Link href={`/subjects/${stat.subject_id}`} key={stat.subject_id}>
              <div className="glass rounded-xl p-6 border-l-4 hover:bg-surface-2/40 transition-all cursor-pointer group h-full flex flex-col justify-between"
                   style={{ borderLeftColor: stat.subject_color }}>
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <span className="inline-block px-2 py-1 rounded bg-surface-2 text-text-muted text-xs font-semibold tracking-widest mb-2">
                        {stat.subject_code}
                      </span>
                      <h4 className="text-lg font-bold text-white group-hover:text-indigo transition-colors line-clamp-2">
                        {stat.subject_name}
                      </h4>
                    </div>
                    <div 
                      className="w-10 h-10 rounded-lg flex items-center justify-center bg-opacity-20 shrink-0"
